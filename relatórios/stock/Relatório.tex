...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}       
        ...
        \node at (2,-1.8) {\texttt{onCreate()}};

        \draw[draw=black] (0.2, -2.7) rectangle (4.8, -3.5) node[pos=.5] {Button};
        \draw[draw=black] (0.2, -3.5) rectangle (4.8, -5.8) node[pos=.5, align=left] {\texttt{setOnClickListener()} \\ \texttt{onClick()} \\ \quad \texttt{new Intent()}};

        \draw[draw=black] (10, 0) rectangle (15, -1) node[pos=.5] {Plot Activity};
        \draw[draw=black] (10, -1) rectangle (15, -6) node[pos=.5, align=left] {\texttt{onCreate()}};

        \draw[draw=black] (10, -8) rectangle (15, -9) node[pos=.5] {FetchData};
        \draw[draw=black] (10, -9) rectangle (15, -14) node[pos=.5, align=left] {\texttt{processJsonData()} \\ \texttt{plotData()}};
        \node at (12.5,-14.5) {Thread};

        \draw[->] (4.8, -4.6) -- (10, -4.6) node[above, midway] {\texttt{startActivity(intent)}};
        \draw[->] (12.5, -6) -- (12.5, -8) node[left, midway] {\texttt{callExtService()}};
        \draw[->] (10, -11) -- (4, -11) node[above, midway] {HTTP Connection};
        \draw[->] (4, -11.5) -- (10, -11.5) node[below, midway] {\texttt{readStream()}};

        % Draw the server body in isometric view (parallelograms)
        \coordinate (A) at (1,-10.2);
